        ...
        while (running) {
            ...
            if(file.startsWith("/Clima")) {
                outputLine = "HTTP/1.1 200 OK\r\n"
                        ...
                        +"<script>"
                        +"const urlApi = 'https://clima1api.herokuapp.com/consulta?lugar=';"
                        +"const ciudadDigitada = document.getElementById('climatextid');  "
                        +"const respuestaCiudad = document.getElementById('obtenerConsultaClimaidiv');"
                        +"async function obtenerConsultaClima(){ \n"
                        +"const answer = await fetchData(ciudadDigitada.value);"
                        +"const resJson = JSON.stringify(answer);"
                        +" ciudadResult.innerHTML = `Resultado: ${resJson}`;"
                        +"}"
